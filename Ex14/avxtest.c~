...
int main(int argc,char **argv){
  ...
  a = (float *)_mm_malloc(sizeof(float)*num_data, 32);
  b = (float *)_mm_malloc(sizeof(float)*num_data, 32);// 1次元配列 a と b のメモリ領域を確保
...
float calc_avx(float c,float *v1, float *v2, int n){ // AVXによるベクトル演算(SIMD)をここで作る
  ...
  t = (float *)_mm_malloc(sizeof(float)*n,32);

  v_c =  _mm256_broadcast_ss(&c);  //スカラー変数 c を ベクトル v_c へ放送する


  num_chunk = n/vec_size;         // １回のベクトル演算でまとめて扱える変数はvec_sizeだから、全部でnum_chunk回のベクトル演算をする
  for(i = 0; i < num_chunk; i++){
    __m256 vec_v1 = _mm256_load_ps(v1+vec_size*i);  // 通常のfloat型配列v1をベクトル vec_v1へロード
    __m256 vec_v2 = _mm256_load_ps(v2+vec_size*i);  // 通常のfloat型配列v2をベクトル vec_v2へロード

    vec_v1 = _mm256_mul_ps(vec_v1, _mm256_mul_ps(_mm256_sqrt_ps(v_c), vec_v2));         //計算式部分。イントリンシック関数を入れ子にすれば１行で書けるが、
      //分けて書いてもよい。必要に応じて、__m256型のベクトル変数を新たに宣言せよ。

    u = _mm256_add_ps(u, vec_v1);// vec_v1の部分的な結果をuに加える
  }
  _mm256_store_ps(t, u);                    // ベクトル u を 通常のfloat型配列 t へストア
