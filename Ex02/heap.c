...
void downheap(int *A, int k, int n){//downheapによってヒープチェック・メンテナンスを行う関数
  ...
  while (k<=n/2) {
    j = k+k;//子ノードへ移る
    if (j<n && A[j]<A[j+1]) j++;   //２つの子ノードのうち、どちらと比較すればいいか
    if (v>=A[j]) break;
...
void myheapsort(int *A, int n){
  ...
  for (i=n; i>=2; i--){
    ...
    downheap(A, 1, i-1);//最大値を根とする構造にするため
  }
