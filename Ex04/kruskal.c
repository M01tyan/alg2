...
int main(){
  ...
  Edge *E;//辺を格納する構造体配列
    ...
    scanf("%d", &n);
    E = (Edge *)malloc(n*n*sizeof(Edge)+1);

    printf("Input the Distance matrix:\n");
    for (i = 0, j = 1; i < n*n; i++){
        scanf("%d", &dist);
        if(dist==INFTY) continue;
        E[j].cost = dist;
...
int Kruskal(Edge *E,int Esize){
  ...
  construct_2(E, Esize);  //辺の集合Eを、辺のコストを基準にminimum heap化する

    while(Esize>0){ //部分木に取り込んでいない辺の数
        minimum = delete_root(E, &Esize);   //heapから最小コストの辺を取り出す

        S1= Find_tree(minimum.v_1);     //最小コストの辺の頂点１はどこに繋がっているか？（どの部分木に属するか）
        S2= Find_tree(minimum.v_2);      //最小コストの辺の頂点２はどこに繋がっているか？（どの部分木に属するか）

        if(S1!=S2){                   //もし、２つの頂点が同じ部分木に属していないのであれば
            Union_tree(minimum.v_1, minimum.v_2);      //その２頂点を結ぶ辺を採用し、部分木同士を結合する
...
void downheap(Edge *A, int k, int n){//downheapによってヒープチェック・メンテナンスを行う関数
  ...
  Edge v;  //一時退避用の変数
    ...
    while (k<=n/2) {
        j=k+k;
        if (j<n && A[j].cost>A[j+1].cost ) j++;   //２つの子ノードのうち、どちらと比較すればいいか
        if (v.cost <= A[j].cost) break;
...
NodePointer insert_node(int value,int source){
  NodePointer newnode,n;
    ...
    for (n=heads[source]; n->next!=NULL; n=n->next);//リスト末尾に接続するためにループ空回し
...
int Union_tree(int a,int b){
  int S1,S2;
    ...
    if(S1==S2) return -1; //無くても問題は無い．（Kruskal()内でどうせ行う）
